...
- Supabase mode has schema, RLS, PostGIS/event/collaboration migrations, setup docs, and basic auth scaffolding, but the full data adapter is not complete beyond auth.
...
- Kakao Maps is documented and abstracted, but the real adapter is not enabled without credentials.
